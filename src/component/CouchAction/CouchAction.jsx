...
import { CouchActionStyle, CollapseStyle } from './CouchAction.style';
    ...
    for (let x = 0; x < memdata.length; x++) {
      ...
      for (let i = 0; i < singleTeam.length; i++) {
        finalMemberData.push({
          ...
          value: singleTeam[i].score_details,
        });
      }
    }
      ...
      for (let i = 0; i < activityName.length; i++) {
        let processedItem = {
          activity: {
            ...
            description: activityName[i].description,
            options: activityName[i].options,
          },
          columns: [],
        };

        for (let x = 0; x < activityName[i].activity_items.length; x++) {
          let descriptionBigArr = [];
          let sectionDescription = activityName[i].activity_items[x].description
            .split('*')
            .join('')
            .split('%%%');
          for (let x = 0; x < sectionDescription.length; x++) {
            descriptionBigArr.push(sectionDescription[x].split('@@@'));
          }
          processedItem.columns.push({
            name: activityName[i].activity_items[x].name,
            id: activityName[i].activity_items[x].id,
            options: activityName[i].activity_items[x].options,
            description: descriptionBigArr,
          });
        }
          ...
          <tbody>
            {allData.map((data, index) => (
              <tr key={data.id}>
                ...
                {updatedActivityData[currentActivity]?.columns.map(
                  (data2, index2) => (
      ...
      <CollapseStyle>
        {allData.map((data, index) => (
          <Collapse
            ...
            data={data.value}
            item={updatedActivityData[currentActivity]?.columns}
            activity_id={updatedActivityData[currentActivity].activity.id}
            member_id={allData[index].id}
          />
        ))}
      </CollapseStyle>
    </>
